...
import { ObjectDetailsViewType } from "../../../sys/types";
import { glob } from '@/main';
const main = require("@/main");
let DetailsView = class DetailsView extends Vue {
    get currentGroup() {
        if (!this._currentGroup)
            this._currentGroup = this.groups[0];
    ...
    groupVisible(group) {
        if (this.groups.length <= 1)
            return false;
        if (this.root)
            return this.dec.detailsViewType == ObjectDetailsViewType.Tabular || this._currentGroup == group;
        else
            return true;
    }
