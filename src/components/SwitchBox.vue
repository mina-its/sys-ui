<template>
      <span class="switch-box">
        <input type="checkbox" :id="`id-${randomId}`" :checked="checked" @keyup.space="changed" @keydown="keydown" @input="changed">
        <label :for="`id-${randomId}`"></label>
      </span>
</template>
...
<script lang="ts">
    ...
    @Component({name: 'SwitchBox'})
    export default class SwitchBox extends Vue {
        @Prop() private checked: boolean;
        private randomId = ID.generateByBrowser().toString().replace(/\./, "");
