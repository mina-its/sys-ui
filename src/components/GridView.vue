<template>
    <div :class="'grid-view' + (root?' p-4':'')" @scroll="onScroll()">
        <div v-if="meta.filter && meta.filter.items" class="p-2 btn-toolbar">
            <filter-item :item="item" :key="item.id" v-for="item in meta.filter.items" :meta="meta"></filter-item>
        </div>
        <table>
            <thead>
            <tr>
            ...
            <tr>
                <td class="border-0" colspan="100">
                    <div class="align-items-center d-flex">
                        ...
                        <ul v-if="this.meta.pages > 1" class="m-2 pagination flex-grow-1">
                            <li class="page-item"><a @click="goBack" href="javascript:;" class="page-link"><i
                                    class="fa fa-chevron-left"></i></a></li>
                            <li v-for="page in meta.pageLinks" :class="'page-item' + (page.active ? ' active':'') "><a
...
<style lang="scss">
    .grid-view {
        ...
        th {
            @extend .p-2;
            font-weight: 500;
            border: 1px solid theme-color("grid-border");
        ...
        thead {
            tr {
                background-color: theme-color("grid-head");
                cursor: pointer;
            }
        }
        ...
        tbody {
            tr {
                ...
                &:hover {
                    background-color: theme-color("grid-row-hover");
                }

                &.selected td:first-child {
                    background-color: theme-color("grid-row-header-selected");
                }

                &.selected td:not(:first-child) {
                    background-color: theme-color("grid-row-selected");
                }
            }
        }
