...
<script lang="ts">
	...
	export default class ElemProp extends Vue {
		...
		render(ce) {
			...
			if (prop(this).condition != null && !main.evalExpression(this.item, prop(this).condition)) {
				this.item[prop(this).name] = null;
				return null;
			}
			...
			focused (e) {
				this.$emit('focus', e, prop(this));
			}

			keydown (e) {
				this.$emit('keydown', e, prop(this));
			}

			renderDetailsView (ce) {
				let valueClass = `prop-value border mx-2`;
				if (prop(this)._.gtype == GlobalType.object && !prop(this).documentView) {
					let oData = st.data[prop(this)._.ref] || [];
					main.vueResetProperties(oData, prop(this)._.ref, false);
					return ce('object-view', {
						props: {root: false, elem: {obj: {ref: prop(this)._.ref, root: false}}},
					});
				}

				let vl = this.renderValue(ce, valueClass);
				let embed: EmbeddedInfo = this.item._ && this.item._[prop(this).name];
				let msg = null;
				if (embed)
					msg = ce('prop-message', {props: {"message": embed.err}});
				let cmt = prop(this).comment ? ce('div', {attrs: {"class": "prop-comment p-4 mt-3"}}, prop(this).comment) : null;
				let title = prop(this).title || prop(this).name;
				let lbl = (main.someProps(prop(this))) ? null : ce('label', {attrs: {"class": "prop-label align-top"}}, title);

				let styles = "form-group";
				if (prop(this)._.gtype == GlobalType.boolean)
			...
			renderTreeView (ce) {
				...
				let cmt = prop(this).comment ? ce('small', {attrs: {"class": "property-comment p-3 text-muted d-block"}}, prop(this).comment) : null;
				let title = prop(this).title || prop(this).name;
				let lbl = this.labelMode == PropertyLabelMode.Hidden ? null : (main.someProps(prop(this))) ? null : ce('label', {attrs: {"class": "mr-1 tree-view-attr-label"}}, title + ":");
			...
			renderValue (ce, styles: string) {
				let pr = {doc: this.item, name: prop(this).name, meta: prop(this), viewType: this.viewType, styles};

				if (prop(this)._.isRef) {
					if (this.viewType == ObjectViewType.TreeView)
						return ce('span', {attrs: {"class": styles + " text-success"}}, main.getPropReferenceValue(prop(this), this.item));
				...
				switch (prop(this)._.gtype) {
					case GlobalType.string:
						if (this.viewType == ObjectViewType.TreeView) {
							let text = main.getPropTextValue(prop(this), this.item) || "";
							return ce('span', {attrs: {"class": styles + " text-success"}}, text.substring(0, 30) + (text.length > 30 ? "..." : ""));
						}

						if (this.viewType != ObjectViewType.GridView && prop(this).text && prop(this).text.multiLine)
							...
							if (this.viewType == ObjectViewType.GridView && this.indexInGrid === 0 && main.getBsonId(this.item)) {
								...
								return ce('prop-text', {
									attrs: {
										type: (prop(this).text && prop(this).text.password) ? 'password' : 'text',
										"class": styles
									},
									on: {changed: this.changed, keydown: this.keydown, focus: this.focused},
									props: pr,
					...
					case GlobalType.number:
						if (this.viewType == ObjectViewType.TreeView)
							return ce('span', {attrs: {"class": styles + " text-success"}}, this.item[prop(this).name]);
				...
				console.error("prop: Invalid property", prop(this));
				return ce('span', '***');
			}
	}
</script>
