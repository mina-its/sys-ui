...
let PropReference = class PropReference extends vue_property_decorator_1.Vue {
    ...
    update(e) {
        ...
        let items = val == null || (this.prop._.items.length < types_2.Constants.contextMenuVisibleItems && val == this.value) ? this.prop._.items : this.prop._.items.filter(item => item.title.toLowerCase().indexOf(val.toLowerCase()) > -1);
