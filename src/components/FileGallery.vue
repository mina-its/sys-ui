    ...
    <div id="file-gallery" class="modal fade" tabindex="-1" role="dialog" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
            <div class="modal-content" @contextmenu="openRootMenu">
                <div class="modal-header" v-if="st.fileGallery.drive">
                ...
                <div class="modal-body">
                    <div class="d-flex flex-wrap p-3">
                        <transition name="fade">
                            <div v-if="st.fileGallery.loading" class="file-gallery-waiting"><i
                ...
                <div class="modal-footer">
                    <div class="d-flex w-100">
                        <function v-if="st.fileGallery.selectable" styles="m-2 btn-primary" @exec="select"
...
<script lang="ts">
	...
	export default class FileGallery extends Vue {
		...
		isSelected(item: DirFile) {
			return st.fileGallery.selected == item;
		}
		...
		select(cn, done, item: DirFile) {
			...
			if (st.fileGallery.selected && st.fileGallery.selected.type == DirFileType.Folder) {
				...
				st.fileGallery.fileSelectCallback(st.fileGallery.path, st.fileGallery.selected);
				if (done) done();
			}
		}
		...
		get breadcrumb() {
			if (st.fileGallery.fixedPath)
				return [];
			let parts = st.fileGallery.path.split("/").filter(el => el);
			let result: Pair[] = [];
			...
			if (result.length > 0)
				result.unshift({title: main.getText(st.fileGallery.drive.title), ref: ""});
			result.pop();
			return result;
		}
		...
		get files() {
			if (st.fileGallery.fixedPath)
				st.fileGallery.list = st.fileGallery.list.filter(f => f.type == DirFileType.File);
			st.fileGallery.list.sort((a, b) => b.type - a.type);
			st.fileGallery.list.forEach(i => i["editing"] = false);
			return st.fileGallery.list;
		}

		get current() {
			if (st.fileGallery.fixedPath) {
				return main.getText(st.fileGallery.drive.title) + ` (${st.fileGallery.path})`;
			} else {
				let parts = st.fileGallery.path.split("/").filter(el => el);
				if (parts.length == 0) return main.getText(st.fileGallery.drive.title);
				return parts.pop();
			}
		}
	}
