...
"{'main-body h-100 d-flex flex-column flex-fill overflow-auto':1, 'root': root}" >
    --Toolbar;
-- >
    v - ;
if ( = "root")
...
-- >
    v - ;
if ( = "root && filter && filteringProp && !dec.filterDec")
    class {
    }
...
"fal fa-plus-circle pr-2" > /i>{{newItem}}</button >
    styles;
"text-secondary fal fa-cog fa-lg";
name = "clickTitlePin";
"clickTitlePin" > /Function>
    < /div>
    < !--Table;
-- >
    class {
...
"{'d-flex w-100 overflow-auto':true, 'bg-white':!root}";
"onScroll()" >
    --Side;
Menu-- >
    v - ;
if ( = "root && dec.filterDec")
    class {
    }
"border-right separator-line bg-white sidenav p-3 d-none d-md-block" >
    class {
    };
...
} = "filter-chip border d-flex align-items-center py-1 px-2 bg-white mr-2" >
; )
...
let GridView = class GridView extends vue_property_decorator_1.Vue {
    ...
    objectFilterChanged(e) {
        ...
        let ref = main_1.setQs(types_2.ReqParams.query, JSON.stringify(this.dec.filterData), true);
    ...
    refreshQueryByFilter() {
        ...
        let ref = main_1.setQs(types_2.ReqParams.query, query ? JSON.stringify(query) : null, true);
        ref = main_1.setQs(types_2.ReqParams.page, null, true, ref);
        main_1.load(ref, true);
    }
    propLocaleName(prop) {
        if (prop.text && prop.text.multiLanguage)
            return prop.name + "." + main_1.glob.config.locale;
        else
            return prop.name;
    }
    removeFilter(prop) {
