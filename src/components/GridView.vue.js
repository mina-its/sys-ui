...
import { ObjectViewType, GridRowHeaderStyle, NewItemMode, LogType, ReqParams, WebMethod, Keys } from '../../../sys/types';
...
let GridView = class GridView extends Vue {
        ...
        for (let i = this.items.length - 1; i >= 0; i--) {
            ...
            if (item._status) {
                ...
                if (!(item._id < 0)) { // not newly added item
                    ...
                    glob.modifies.push({ type: WebMethod.del, ref: url });
                    throw 'todo';
    ...
    deselectAll(but) {
        this.items.forEach(item => item._.marked = null);
        if (but)
            but._.marked = true;
    }
    rowSelected(item) {
    ...
    rowMove(up) {
        let item = this.items.find(item => item._.marked = true);
...
__decorate([
    Prop()
], GridView.prototype, "items", void 0);
GridView = __decorate([
    Component({
