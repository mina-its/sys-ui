...
let GridView = class GridView extends vue_property_decorator_1.Vue {
    ...
    onUriReset() {
        ...
        main_1.glob.infoPanel.currentComment = this.dec.comment;
    }
    checkForAddButton() {
        this.addButton = (this.dec.access & types_2.AccessAction.NewItem) && this.dec.newItemMode == types_2.NewItemMode.newPage;
    }
    clickRecentItem(item) {
        main_1.pushToGridViewRecentList(location.pathname.replace(/^\//, ""), item.ref, item.title);
    }
    resetRecentItems() {
    ...
    resetGlobalFuctions() {
        ...
        if (this.dec.links) {
            this.globalFuctions = this.dec.links.filter(link => !link.disable && !link.type).map(link => {
                return { title: link.title, ref: main_1.prepareServerUrl(link.address, true) };
            });
        }
    }
    mounted() {
    ...
    clickNewItem() {
        let ref = main_1.setQs(types_2.ReqParams.newItem, "1", false, location.pathname);
        main_1.load(ref, true);
    }
    clickObjectMenu(e) {
        let items = [
        ...
        main.showCmenu(null, items, e, (state, item) => {
            switch (item.ref) {
                case "export-csv":
                    main_1.call('exportData', { type: types_2.FileType.Csv }, (err, res) => {
                    });
                    break;
                default:
                    main_1.notify('File type not supported yet', types_2.LogType.Error);
                    break;
                case "print":
                    window.print();
                    break;
    ...
    insert() {
        switch (this.dec.newItemMode) {
            case types_2.NewItemMode.newPage:
                let ref = main_1.setQs(types_2.ReqParams.newItem, "1", false, this.uri);
                ref = main_1.prepareServerUrl(ref, true);
                if (this.dec.newItemDefaults)
                    ref = main_1.setQs(types_2.ReqParams.newItemDefaults, this.dec.newItemDefaults, false, ref);
                main.load(ref, true);
                break;
            default:
                if (main_1.getQs(types_1.Constants.QUERY_NEW)) {
                    main_1.notify("Please save your changes before!", types_2.LogType.Warn);
                    return;
                }
                let newItem = { _id: types_1.ID.generateByBrowser(), _new: true, _: { marked: false, dec: this.dec } };
                if (this.dec.newItemDefaults) {
                    let defaults = bson_util_1.parse(this.dec.newItemDefaults, true, types_1.ID);
...
GridView = tslib_1.__decorate([
    vue_property_decorator_1.Component({ name: 'GridView' })
], GridView);
