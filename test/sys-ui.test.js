...
async function click(page, selector, innerHTML) {
    if (innerHTML) {
...
async function setProperty(selector, propName, value) {
...
async function getProp(selector, propName) {
...
async function submitRecord(row, disabled, name, age, birthday, language, codes) {
    ...
    if (birthday == "today") {
        //    click on calender
        await page.waitFor(`tbody>:nth-child(${row})>:nth-child(6)>div>div>div>i`);
        await click(page, `tbody>:nth-child(${row})>:nth-child(6)>div>div>div>i`);
        //    set date
        await page.waitFor(`div.date-time-picker>:nth-child(2)>button`);
        await click(page, `div.date-time-picker>:nth-child(2)>button`, 'Set');
    }
    else
        await page.type(`tbody>:nth-child(${row})>:nth-child(6)>div>div>input`, birthday);
    // add age property
    await page.type(`tbody>:nth-child(${row})>:nth-child(5)>input`, age.toString());
    // add name property
    await page.waitFor(`tbody>:nth-child(${row})>:nth-child(4)>input`);
    await page.type(`tbody>:nth-child(${row})>:nth-child(4)>input`, name);
    // select Language
    await click(page, `tbody>:nth-child(${row})>:nth-child(7)>input`);
    await page.waitFor('a.dropdown-item');
    await click(page, 'a.dropdown-item', language);
    // select codes
    for (let code of codes) {
        await page.waitFor(`tbody>:nth-child(${row})>:nth-child(8)>div>:last-child`);
        await click(page, `tbody>:nth-child(${row})>:nth-child(8)>div>:last-child`);
...
async function emptyRecord(row) {
    ...
    // add age property
    await set(`tbody>:nth-child(${row})>:nth-child(5)>input`, "");
    // add birthday property
    await set(`tbody>:nth-child(${row})>:nth-child(6)>div>div>input`, "");
    // select Language
    await set(`tbody>:nth-child(${row})>:nth-child(7)>input`, "");
    // empty codes
    let codeElements = await page.$$eval(`tbody>:nth-child(${row})>:nth-child(8)>div> *  `, (codes) => {
        codes.forEach(code => {
...
async function changeRecord(row, disabled, name, age, birthday, language, codes) {
}
async function checkCorrectness(row, disabled, name, age, birthday, language, codes) {
    //  property disabled
    ...
    // property name
    let nameReal = await getProp(`tbody>:nth-child(${row})>:nth-child(4) input`, 'value');
    expect(nameReal).toBe(name);
    // property date
    let birthdayReal = await getProp(`tbody>:nth-child(${row})>:nth-child(6)>div>div>input`, 'value');
    if (birthday == "today")
        expect(birthdayReal).toBe(moment(new Date()).format('YYYY/MM/DD').toString());
    else
        expect(birthdayReal).toBe(birthday);
    // property age
    let ageReal = await getProp(`tbody>:nth-child(${row})>:nth-child(5) input`, 'value');
    expect(ageReal).toBe(age.toString());
    // property language
    let languageReal = await getProp(`tbody>:nth-child(${row})>:nth-child(7) input`, 'value');
    expect(languageReal).toBe(language);
    // property codes
    for (let index in codes) {
        let codeReal = await page.$eval(`tbody>:nth-child(${row})>:nth-child(8)>div>:nth-child(${(parseInt(index) + 1)}) span`, el => el.innerHTML);
...
async function deleteRecord(page, type, row) {
    if (!row)
        row = 1;
    await page.waitFor('i.m-1.fal.fa-plus-circle');
...
async function saveRefresh(page) {
    ...
    await page.waitFor('button > span');
    await click(page, 'button > span', 'Apply');
    await waitForSelector(page, '#snackbar.visible', 'Saved!');
    //refresh
...
describe('e2e crud test', () => {
    ...
    test('login', async () => {
        await click(page, 'a[href="/signin"]');
    ...
    test('Loading testObject', async () => {
        await page.waitFor('a[href="/testObject"]');
        await click(page, 'a[href="/testObject"]');
    });
    // test('insert new document', async () => {
    //   await deleteRecord(page,'All');
    //
    //   // click plus
    //   await waitForSelector(page,'i.m-1.fal.fa-plus-circle');
    //   await click(page,'i.m-1.fal.fa-plus-circle');
    //
    //   //insert row 1
...
describe('Security check', function () {
    test('Goto objects in new page', async () => {
        ...
        await page2.goto('http://localhost:80/', { waitUntil: 'networkidle0' });
        //login
        await click(page2, 'a[href="/signin"]');
        ...
        await page2.type('input.filter-prop-value[name="title"]', 'test object');
        await page2.keyboard.press('Enter');
        // select test object
        await page2.waitFor('a[href="/objects/5e6228db4cbdf5495c905b75"]');
        await click(page2, 'a[href="/objects/5e6228db4cbdf5495c905b75"]');
    });
...
//# sourceMappingURL=sys-ui.test.js.map